...
class AddnewComplexVoucher extends Controller
{
    ...
    public function addnew(Request $request){
        ...
        $nrtotal = \DB::table('profiles')
            ...
            ->select(
                'subscriptions.no_complex_vouchers as nrTotal'
            )->value('nrTotal');
        ...
        if ($request['type'] == 1) {
            $this->validate($request, [
                ...
                'steps' => 'required|integer|max:5|min:1',
                'couponcode.*' => 'required',
                'category' => 'required',
                'location' => 'required',
        ...
        $type = $request['type'];
        $couponcode = serialize($request['couponcode']);
        ...
        return redirect('admin/addnewComplex');
}
}
